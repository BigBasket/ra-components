...
export const TreeMenu = (props) => {
    ...
    const pathname = '/users'; //useSelector((state) => state.router.location.pathname);
    let allResources = useResourceDefinitions();
    const resources =  [];
    for (var resource in allResources)
        resources.push(allResources[resource]);
    ...
    resources && resources.length>0 && resources.forEach(resource => {
