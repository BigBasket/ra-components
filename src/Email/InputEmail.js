...
export const EmailInput = (props) => {
  ...
  const checkEmailDomain = (value) => {
    ...
    if (name && domain && domains.includes(domain)) return true;
    return false;
  };
  ...
  const sx = wrap
    ? { display: "inline", flexDirection: "row", flexWrap: "wrap" }
    : undefined;
  ...
  if (type === EmailTypes.ARRAY) {
    ...
    retComponent = <TextInput source={source} validate={validate} {...rest} />;
